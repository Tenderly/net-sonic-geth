...
func testTwoOperandOp(t *testing.T, tests []TwoOperandTestcase, opFn executionFunc, name string) {
	...
	for i, test := range tests {
		...
		stack.push(x)
		stack.push(y)
		opFn(&pc, evm.interpreter.(*EVMInterpreter), &ScopeContext{nil, stack, nil})
		if len(stack.data) != 1 {
...
func TestAddMod(t *testing.T) {
	...
	for i, test := range tests {
		...
		stack.push(y)
		stack.push(x)
		opAddmod(&pc, evm.interpreter.(*EVMInterpreter), &ScopeContext{nil, stack, nil})
		actual := stack.pop()
		if actual.Cmp(expected) != 0 {
...
func opBenchmark(bench *testing.B, op executionFunc, args ...string) {
	...
	for i := 0; i < bench.N; i++ {
		...
		op(&pc, evm.interpreter.(*EVMInterpreter), scope)
		stack.pop()
	}
	bench.StopTimer()
...
func TestOpMstore(t *testing.T) {
	...
	opMstore(&pc, evm.interpreter.(*EVMInterpreter), &ScopeContext{mem, stack, nil})
...
func TestOpTstore(t *testing.T) {
	...
	stack.push(new(uint256.Int))
	opTstore(&pc, evm.interpreter.(*EVMInterpreter), &scopeContext)
	...
	// push the location to the stack
	stack.push(new(uint256.Int))
	opTload(&pc, evm.interpreter.(*EVMInterpreter), &scopeContext)
...
func BenchmarkOpKeccak256(bench *testing.B) {
	...
	for i := 0; i < bench.N; i++ {
		...
		stack.push(start)
		opKeccak256(&pc, evm.interpreter.(*EVMInterpreter), &ScopeContext{mem, stack, nil})
	}
}
...
func TestRandom(t *testing.T) {
	...
	for _, tt := range []testcase{
		...
		opRandom(&pc, evm.interpreter.(*EVMInterpreter), &ScopeContext{nil, stack, nil})
		if len(stack.data) != 1 {
...
func TestBlobHash(t *testing.T) {
	...
	for _, tt := range []testcase{
		...
		opBlobHash(&pc, evm.interpreter.(*EVMInterpreter), &ScopeContext{nil, stack, nil})
		if len(stack.data) != 1 {
			t.Errorf("Expected one item on stack after %v, got %d: ", tt.name, len(stack.data))
		}
		actual := stack.pop()
...
func TestOpMCopy(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		// Do the copy
		opMcopy(&pc, evm.interpreter.(*EVMInterpreter), &ScopeContext{mem, stack, nil})
...
func TestOpCLZ(t *testing.T) {
	...
	for _, tc := range tests {
		...
		stack.push(val)
		interpreter, ok := evm.interpreter.(*EVMInterpreter)
		if !ok {
			t.Fatal("interpreter is not EVMInterpreter")
		}
		opCLZ(&pc, interpreter, &ScopeContext{Stack: stack})
